// @ts-nocheck
/**
 * Adapted from webgl-splash-cursor's SplashCursor.jsx for framework-free use.
 */
export function initFluidInk(canvas, {
      SIM_RESOLUTION = 128,
      DYE_RESOLUTION = 1440,
      ...
      ENABLE_INITIAL_SPLATS = false
        ...
        const displayShaderSource = `
          ...
          void main () {
              ...
              float maskAlpha = 1.0 - clamp(a * 40.0, 0.0, 1.0);
              gl_FragColor = vec4(vec3(0.9608) * maskAlpha, maskAlpha);
          }
        `;
        ...
        function updatePointerMoveData(pointer, posX, posY, color) {
          ...
          pointer.texcoordX = posX / canvas.width;
          pointer.texcoordY = 1.0 - posY / canvas.height;
          pointer.deltaX = correctDeltaX(pointer.texcoordX - pointer.prevTexcoordX) * 10.0;
          pointer.deltaY = correctDeltaY(pointer.texcoordY - pointer.prevTexcoordY) * 10.0;
        ...
        // 命名事件处理函数，确保卸载时能正确移除监听。
        function handleMouseDown(e) {
          let pointer = pointers[0];
          const bounds = canvas.getBoundingClientRect();
          let posX = scaleByPixelRatio(e.clientX - bounds.left);
          let posY = scaleByPixelRatio(e.clientY - bounds.top);
        ...
        function handleTouchStart(e) {
          ...
          for (let i = 0; i < touches.length; i++) {
            let posX = scaleByPixelRatio(touches[i].clientX - bounds.left);
            let posY = scaleByPixelRatio(touches[i].clientY - bounds.top);
